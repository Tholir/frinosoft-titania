...
export const rulesEngine = {
    ...
    calculateHP(classes, totalLevel, conMod) {
    ...
    calculateSkillBonus(skillName, character) {
    ...
    getSpellSlots(level, spellcastingAbility) {
    ...
    getAvailableFeats(level) {
        if (level < 4)
            return [];
        if (level < 6)
            return ["Actor", "Alert", "Athlete", "Defensive Duelist", "Dual Wielder", "Dungeon Delver",
    ...
    validateAbilityScores(abilities, method, rolls) {
        ...
        if (method === "standard_array") {
            ...
            if (!match)
                errors.push("Scores don't match the Standard Array");
        }
            ...
            if (cost > 27)
                errors.push(`Point buy cost ${cost} exceeds maximum of 27`);
        }
        else if (method === "roll" && rolls) {
            if (rolls.length !== 6)
                errors.push("Must provide exactly 6 rolls");
            for (const r of rolls) {
                if (r < 3 || r > 18)
                    errors.push(`Roll ${r} is outside valid range (3-18 for 3d6)`);
            }
        }
